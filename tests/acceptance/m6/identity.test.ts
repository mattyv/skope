...
// failure until the host stamps skope_version/skope_build for real.
...
describe("M6: one build identity everywhere (SPEC §7.2)", () => {
  test("run_start's skope_version/skope_build match `skope --version`", async () => {
    const version = await runSkope(["--version"]);
    const m = version.stdout.match(/^skope (\S+) \(build identity ([0-9a-f]{64})\)/);
    expect(m).not.toBeNull();
    const [, releaseVersion, buildIdentity] = m as unknown as [string, string, string];

    const run = await runSkope([SKILL, "--apply", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
    const start = run.events[0] as { event: string; skope_version: string; skope_build: string };
    expect(start.event).toBe("run_start");
    expect(start.skope_version).toBe(releaseVersion);
    expect(start.skope_build).toBe(buildIdentity);
  });
});
