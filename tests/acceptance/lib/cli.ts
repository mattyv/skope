...
// goes through this, so there's one place that knows how skope is invoked.
...
/** A parsed line of skope's JSON Lines output (SPEC §10); shape varies by `event`. */
export type SkopeEvent = { event: string } & Record<string, unknown>;
...
// lock at $XDG_RUNTIME_DIR/skope/<name>.lock (SPEC §7 step 3), and never leak state between
...
function cleanEnv(args: string[], extra: Record<string, string> | undefined): Record<string, string> {
  const root = mkdtempSync(join(tmpdir(), "skope-test-"));
  ...
  delete env.SKOPE_CALLER;
  // XDG_RUNTIME_DIR, when set, is a directory that exists (XDG Base Directory spec); skope doesn't create it.
  mkdirSync(runtimeDir, { mode: 0o700 });

  if (!args.includes("--config")) {
    mkdirSync(join(configHome, "skope"), { recursive: true });
    writeFileSync(
      join(configHome, "skope", "config.yaml"),
      `pager:\n  command: "cat > /dev/null"\nstate_dir: ${stateHome}/skope\nask:\n  backend: fake\n`,
    );
  }
...
export function runSkope(args: string[], opts: { env?: Record<string, string>; input?: string } = {}): Promise<CliResult> {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      const events: SkopeEvent[] = stdout
        .split("\n")
        .filter(Boolean)
        .flatMap((l) => {
          try {
            return [JSON.parse(l) as SkopeEvent];
          } catch {
