...
describe("M2: --verify (SPEC §5.4, §5.6)", () => {
  test("terminates on disk-full and reports a finite, non-empty set of paths", async () => {
    const report = await verify(DISK_FULL);
    expect(report.paths).toBeGreaterThan(0);
  });

  test("terminates on cert-expiry and reports a finite, non-empty set of paths", async () => {
    const report = await verify(CERT_EXPIRY);
    expect(report.paths).toBeGreaterThan(0);
  });

  test("every path ends in a real outcome (stopped, paged or handoff) — never falls through, never 'error' (P6)", async () => {
    const report = await verify(DISK_FULL);
  ...
  test("reports max backend (ask) calls on any path", async () => {
    const report = await verify(DISK_FULL);
    expect(report.max_ask_calls).toBeGreaterThan(0);
  });

  test("disk-full's Clean up loop is 5 items, bounded: reflected in a finite max_effects", async () => {
    const report = await verify(DISK_FULL);
