...
// agent launched. A handoff under --apply pages; --no-page, SKOP_CALLER=agent
...
describe("M5: handoff (SPEC §8)", () => {
  test.fails("a handoff run exits 20", async () => {
    const r = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
    expect(r.code).toBe(20);
  });

  test.fails("the handoff_record event carries path and record, and the outcome is 'handoff'", async () => {
    const r = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
  ...
  test.fails("the handoff record carries the standard preamble verbatim (SPEC §8.2)", async () => {
    const r = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
  ...
  test.fails("--apply with no opt-out pages a human on handoff", async () => {
  ...
  test.fails("--no-page stops the handoff page", async () => {
    const r = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
  ...
  test.fails("SKOP_CALLER=agent stops the handoff page", async () => {
    const r = await runSkop([SKILL, "--apply", "--fake", ANSWERS, "--fake-exec", COMMANDS], { env: { SKOP_CALLER: "agent" } });
    expect(r.code).toBe(20); // pins a real handoff run, not just the absence of handoff_page below
    expect(r.events.some((e: { event: string }) => e.event === "handoff_page")).toBe(false);
  });

  test.fails("caller in run_start is 'agent' when SKOP_CALLER=agent, 'person' otherwise", async () => {
    const asAgent = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS], {
      env: { SKOP_CALLER: "agent" },
    });
    const asPerson = await runSkop([SKILL, "--apply", "--no-page", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
  ...
  test.fails("on_handoff: none in config stops the handoff page", async () => {
    const config = tempConfig("on_handoff: none\nask:\n  backend: fake\n");
    const r = await runSkop([SKILL, "--apply", "--config", config, "--fake", ANSWERS, "--fake-exec", COMMANDS]);
    expect(r.code).toBe(20); // pins a real handoff run, not just the absence of handoff_page below
    expect(r.events.some((e: { event: string }) => e.event === "handoff_page")).toBe(false);
  });

  test.fails("dry run logs would_page instead of paging, and the outcome is still handoff", async () => {
    const r = await runSkop([SKILL, "--dry-run", "--fake", ANSWERS, "--fake-exec", COMMANDS]);
  ...
  test.fails("a pager failure is logged but doesn't change the outcome (still handoff, exit 20)", async () => {
  ...
  test.fails("handoff never happens by guessing whether a terminal is attached (piped stdio, no flags, still pages)", async () => {
    // runSkop always pipes stdio (never a tty); the page still fires because nothing said not
